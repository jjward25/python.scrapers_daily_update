...
def appolitics_scrape():
    ## Date List created with string values for the last 4 days to only pull opinions from that range.  
    ## General templates to pull from, not all are always used.
    headers = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.85 Safari/537.36 Edg/90.0.818.46'}
    link = "https://apnews.com/hub/politics?utm_source=apnewsnav&utm_medium=navigation"
    ...
    try:
        ...
        print('URL Broken')
        obj_list = [{'type':'Headline','source':'AP Politics', 'title': 'Link Broken', 'link': '', 'Notes': '', 'date': ''}]
    ...
    for item in soup.find_all(class_='CardHeadline'):
        ...
        obj_data = {'type':'Headline','source':'AP Politics', 'title': title, 'link': ilink, 'Notes': 'notes', 'date': 'idate'}
        object_list.append(obj_data)

    ## Final dataframe is defined
    appolitics_df = pd.DataFrame(object_list).head(8)
