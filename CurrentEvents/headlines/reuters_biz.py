...
def reutersbiz_scrape():
    ...
    try:
        ...
        print('URL Broken')
        obj_list = [{'type':'Headline','source':'Reuters Biz', 'title': 'Link Broken', 'link': '', 'Notes': '', 'date': ''}]
    ...
    for item in soup.find_all(class_='story-card'):
        ...
        obj_data = {'type':'Headline','source':'Reuters Biz', 'title': title, 'link': ilink, 'Notes': 'notes', 'date': 'idate'}
    ...
    if len(reuters_biz_df) == 0:
        print('URL Broken')
        obj_list = [{'type':'Headline','source':'Reuters Biz', 'title': 'Data List Empty', 'link': '', 'Notes': '', 'date': ''}]
        reuters_biz_df = pd.DataFrame(obj_list)
        return reuters_biz_df
