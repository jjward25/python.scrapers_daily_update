...
## Actual HTML pull
object_list = []
for item in soup.find_all(class_="DBQmFf NclIid BL5WZb Oc0wGc xP6mwf j7vNaf"):

    #if item.find('span').get_text() in date_list:

    title = item.find(class_="xBbh9").get_text()
    ilink = "https://www.google.com" + item.find(class_="DY5T1d RZIKme").get('href').lstrip('.')
    notes = item.find()
    idate = item.find()
